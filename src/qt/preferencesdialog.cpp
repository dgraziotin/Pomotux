...
void PreferencesDialog::on_buttonBox_accepted()
{
    ...
    try {
        ...
        string soundActivatedString = soundActivated.mValue;
        bool soundActivatedBool = (bool) atoi(soundActivatedString);
        if (soundActivatedBool != this->m_ui->soundActivated->isChecked()) {
            if (this->m_ui->soundActivated->isChecked()==true)
                soundActivated.mValue = "1";
            else
...
void PreferencesDialog::RefreshSettings()
{
    ...
    try {
        Settings soundActivated = select<Settings>(*(this->mpDatabase), Settings::MName=="soundActivated").one();
        this->m_ui->soundActivated->setChecked((bool)atoi(soundActivated.mValue));
    } catch (NotFound e) {
        Settings soundActivated(*(this->mpDatabase));
        soundActivated.mName= "soundActivated";
        soundActivated.mValue = "0";
